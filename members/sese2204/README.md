# sese2204

## 소개

- GitHub: [@sese2204](https://github.com/sese2204)
- 스터디 목표:
  - Knowledge Graph의 데이터 모델과 저장/검색 방식 이해
  - 문서에서 트리플을 추출해 그래프로 쌓는 파이프라인 직접 구현

## 진행 현황

### notes

| # | 주제 | 상태 |
|---|------|------|
| 01 | [고전 RAG 기술 변천사](notes/01-rag-history.md) | 완료 |
| 02 | [청킹 전략](notes/02-chunking.md) | 완료 |
| 03 | [임베딩 모델 비교](notes/03-embedding-models.md) | 완료 |
| 04 | [하이브리드 검색 (BM25 + 벡터)](notes/04-hybrid-search.md) | 완료 |
| 05 | [리랭커](notes/05-reranker.md) | 완료 |
| 06 | [쿼리 재작성](notes/06-query-rewriting.md) | 완료 |
| 07 | [RAG 평가와 실습 로드맵](notes/07-rag-evaluation.md) | 완료 |

### labs

| # | 실습 | 상태 |
|---|------|------|
| 01 | [카카오톡 대화 파싱 → PostgreSQL 적재](labs/01-kakao-ingest/README.md) | 완료 |
| 02 | [대화 청킹 → 로컬 임베딩 → pgvector / pg_trgm 하이브리드 검색](labs/02-kakao-chunk-embed/README.md) | 완료 |
