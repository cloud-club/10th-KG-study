#!/usr/bin/env python3
"""BM25와 벡터 검색 결과를 점수 정규화 방식으로 융합해 Hit@5·Recall@5를 기록한다.

RRF(순위 융합)와 비교하기 위한 대안 구현. run_hybrid_queries.py와 같은 후보 풀에서
출발하되, 등수 대신 점수를 0~1로 정규화해서 가중합으로 섞는다.
"""
...
DEFAULT_OUTPUT = ROOT / "data" / "do-dop" / "company-analysis-kg" / "results" / "score_fusion.jsonl"
...
def main() -> None:
    ...
    with psycopg.connect(args.dsn) as conn:
        for query in queries:
            ...
            bm25_pairs = [(item["id"], item["score"]) for item in bm25_results]
            ...
            vector_pairs = [(item["id"], item["score"]) for item in vector_results]

            fused = normalized_score_fusion(bm25_pairs, vector_pairs, bm25_weight=args.bm25_weight)[: args.size]
