...
- 답변 생성 LLM: **로컬 Ollama 0.34.0** + `exaone3.5:7.8b`(Q4_K_M, 4.8 GB, LG AI Research 한·영 이중언어)를 `num_ctx 8192`·`temperature 0`으로 감싼 `kakao-rag` 모델(`ollama/Modelfile.exaone`). Apple M4 16 GiB에서 Colima 6 GiB와 함께 돌렸다. `.env`의 `LLM_PROVIDER=none`이면 검색·평가·컨텍스트 조립까지만 수행하고, OpenAI / Gemini도 붙일 수 있다. → `.env.example`
- 외부 키: 없음. 이 문서의 결과는 임베딩·검색·답변 생성 모두 노트북 안에서만 계산했다.
...
### 에이전트 v1 — 답변 생성 평가 (2026-09-16, 로컬 `kakao-rag`)

34문항 전체에 답을 생성하고(`generate_answers.py`, 총 11분 50초, 문항당 p50 19.6초) 기준 답과 인용 청크 본문을 대조해 채점했다. 전체 표는 [`results/generation-summary.md`](results/generation-summary.md). 채점은 이 세션에서 Claude가 1차로 했고 사용자 검토 전이다.

| 지표 | 값 | 뜻 |
|---|---:|---|
| 답변 정확도 (답 있는 30문항) | **0.583** | 0 / 0.5 / 1 평균. exact 0.750 · semantic 0.500 · hard 0.417 |
| 인용 precision | 0.807 | 붙인 `[C#]` 중 실제로 그 문장을 지지한 비율 |
| 인용 recall | 0.774 | 사실 문장 중 인용으로 지지된 비율 |
| 답 없음 거절 성공률 (4문항) | 0.500 | 전화번호·같은 회사는 올바르게 거절, 부산·제주는 미흡 |
| 미제공 인용 ID | 0건 | `[C9]`류 환각 인용 없음. 구조 검증은 34문항 모두 통과 |
| 검색 성공 · 생성 실패 | **12건** | gold가 컨텍스트에 있었는데 정확도 < 1 |
| 검색 실패 | 2건 | gold가 top-5에 없음 (q020·q023, 노트 04의 표·목록형 질문) |

읽는 법:

- **검색이 병목이 아니었다.** 정확도를 깎은 30문항 중 검색 실패는 2건뿐이고 12건은 근거를 손에 들고도 답을 못 냈다. 유형은 [`results/failures.md`](results/failures.md) 2절에 정리했다. 가장 많은 것은 **과잉 거절**(정답 근거를 인용해 놓고 “확인할 수 없습니다”로 마무리, 3건)과 **컨텍스트 무시**(C2에 “매월 18일 이체”가 있는데 다른 청크로 일반화, 2건)였다.
- **인용은 대체로 정직했다.** precision 0.81, 환각 인용 0건. 틀린 인용의 대부분은 “관련은 있으나 그 문장을 지지하지는 않는” 느슨한 첨부였고, 발화자를 바꿔 귀속한 경우가 1건 있었다.
- **시간 축이 없다.** 아웃바운드 요금 질문에서 2023-12·2024-07·2024-09의 서로 다른 요금 이슈 3건을 한 사건으로 합쳤고, 판교 일정 질문은 두 회차(9시·10시) 중 기준 답과 다른 쪽을 답했다. 두 번째는 질문 설계 문제이기도 하다. 청크 헤더에 날짜가 있어도 모델이 그것을 구분 축으로 쓰지는 않았다.
- **집계 3문항은 정직하게 거절했다**(정확도 0이지만 환각 없음). 답 없음 4문항 중 2건은 거절하면서도 없는 연결을 덧붙였다(“워크숍 관련 대화 중 403호”). 거절 성공률 0.5는 “없다”를 말하는 능력이 아직 절반이라는 뜻이다.
- 다중 홉 bridge 질문(요청→반영)은 두 근거가 모두 컨텍스트에 들어왔지만 생성이 요청자를 반영자로 섞었다. 검색은 통과하고 관계 방향에서 실패한 사례다.
- `--dry-run`과 `tests/test_agent.py`로 생성 경로를 stub으로 먼저 검증한 뒤 실제 모델을 붙였다.
...
- [x] Ollama `kakao-rag`(exaone3.5:7.8b)로 34문항 답변 생성·채점·집계 (2026-09-16). 남은 일: `results/private/grading.csv`의 1차 채점을 사용자가 검토해 확정한다.
- [ ] 과잉 거절 3건·컨텍스트 무시 2건에 대해 프롬프트 규칙 4(“근거가 부족하면 거절”)의 문구를 조정하고 같은 34문항으로 재측정한다. 규칙을 바꿀 때마다 평가셋은 그대로 둔다.
- [ ] 더 큰 로컬 모델(exaone3.5:32b는 19 GB라 이 노트북에서는 불가)이나 qwen3:8b(thinking 끄고)와 비교한다. 비교 축은 정확도보다 “검색 성공·생성 실패” 건수.
- [ ] 클라우드 모델을 쓸 경우에만 가명화 누락(제3자 실명) 재처리가 선행돼야 한다.
