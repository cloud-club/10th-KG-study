"""grep / BM25 / pgvector / Hybrid 를 전체로 돌려 Hit@3, MRR을 비교한다.
...
def run_method(
    ...
    if name == "vector":
        ...
        return ranked_source_paths([doc for doc, _ in rows])
    if name == "hybrid":
        rows, _ = search_hybrid(
            query,
            limit=POOL,
            candidate_limit=max(POOL, hybrid_candidate_limit),
            rank_constant=hybrid_rank_constant,
...
def evaluate(
    ...
    index_name: str = ES_INDEX,
    table_name: str = PG_TABLE,
    methods: tuple[str, ...] = ("grep", "bm25", "vector", "hybrid"),
    on_progress=None,
    hybrid_candidate_limit: int = 20,
    hybrid_rank_constant: int = 60,
) -> list[dict[str, Any]]:
...
def main() -> None:
    print(f"질문 {len(load_queries())}건을 grep / BM25 / pgvector / Hybrid 로 실행합니다 (pgvector는 OpenAI 호출)...")
    results = evaluate()
    summarize(results)
